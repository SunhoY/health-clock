...
* 생성 모드 운동 목록은 인증 모드/게스트 모드 모두 서버 API(`GET /api/exercises/body-parts/:bodyPartId/exercises`) 조회 결과를 사용한다
...
* 편집 모드에서 `[삭제]`는 실행 모드별 저장소에서 성공했을 때 목록에서 제거된다
  * 인증 모드: API(`DELETE /api/routines/:routineId/exercises/:routineExerciseId`)
  * 게스트 모드: 로컬 스토리지
